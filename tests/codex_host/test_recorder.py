"""Recorder tests — env-gated, off by default, always redacts (spec §4 + C-6)."""
...
from trowel_py.codex_host.recorder import RECORDER_ENV_FLAG, RawRecorder, recording_enabled


def test_recording_disabled_by_default(monkeypatch, tmp_path: Path) -> None:
    """No env flag → recorder is a no-op."""
...
def test_recording_enabled_by_boolean_flag(monkeypatch, tmp_path: Path) -> None:
    """``TROWEL_CODEX_RECORD=1`` turns the recorder on."""
...
def test_recording_scoped_to_specific_path(monkeypatch, tmp_path: Path) -> None:
    """A path value enables only that one recorder, not its neighbours."""
...
def test_record_redacts_before_write(monkeypatch, tmp_path: Path) -> None:
    """A secret in the message never reaches the JSONL file."""
