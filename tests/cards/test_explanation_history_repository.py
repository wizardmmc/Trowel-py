...
def test_find_by_card_id_orders_oldest_first(db_connection: sqlite3.Connection):
    ...
    db_connection.execute(
        "UPDATE card_explanation_history SET created_at = '2026-01-01 10:00:00' WHERE id = 'h-1'"
    )
    repo.create(_entry("h-2", explanation="new"))
    db_connection.execute(
        "UPDATE card_explanation_history SET created_at = '2026-01-02 10:00:00' WHERE id = 'h-2'"
    )
