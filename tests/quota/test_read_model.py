"""QuotaReadModel behaviour (slice-093-pre, criteria 4-stale + 5)."""
...
def test_ok_snapshot_goes_stale_after_threshold_and_keeps_windows() -> None:
    ...
    clock[0] = 1_000 + 600_001  # past the threshold
    got = rm.get(Provider.GLM, "a")
    assert got is not None
    assert got.status is QuotaStatus.STALE
    assert len(got.windows) == 1  # value still visible, just flagged
...
def test_mark_stale_flips_ok_to_stale_and_keeps_windows() -> None:
    ...
    assert got is not None
    assert got.status is QuotaStatus.STALE
    assert len(got.windows) == 1  # value still visible


def test_mark_stale_leaves_error_and_missing_alone() -> None:
    rm = QuotaReadModel(now_ms=lambda: 0)
    rm.update(_err(Provider.CODEX, "codex", fetched_at=0, status=QuotaStatus.AUTH_ERROR))

    rm.mark_stale(Provider.CODEX, "codex")  # not OK -> no-op
    rm.mark_stale(Provider.CODEX, "missing")  # absent -> no-op
...
def test_all_is_safe_under_concurrent_writer() -> None:
    """all() snapshots keys so an off-loop writer can't crash the reader
    (review HIGH-2: sync route in a threadpool raced the event-loop writers)."""
    ...
    try:
        for _ in range(2000):
            rm.all()  # must not raise "dictionary changed size during iteration"
    finally:
        stop.set()
        thread.join()
