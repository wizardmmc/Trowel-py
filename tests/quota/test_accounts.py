"""GLM account config loading (slice-093-pre)."""
...
class _FakeConfig:
    """Minimal stand-in for LLMConfig (only the two attrs the loader reads)."""
...
def test_placeholder_key_returns_empty() -> None:
    assert (
        load_glm_accounts(
            _FakeConfig(
                "https://open.bigmodel.cn/api/anthropic", "<your-bigmodel-key>"
            )
        )
        == []
    )
...
def test_real_bigmodel_key_returns_one_account_with_host_stripped() -> None:
    ...
    assert accounts[0].host == "https://open.bigmodel.cn"  # /api/anthropic stripped


def test_non_bigmodel_base_url_returns_empty() -> None:
    assert (
        load_glm_accounts(_FakeConfig("https://api.openai.com/v1", "sk-x")) == []
    )


def test_deceptive_lookalike_hostname_is_rejected() -> None:
    """A substring match would leak the raw key to a lookalike host."""
