"""验证 CC proxy 配置增量、provider 门禁与 system rewrite。"""
...
def _p_body() -> dict:
    """已确认的 CC -p system 前缀形态。"""
    return {
        "model": "glm-5.1",
        "system": [
            {
                "type": "text",
                "text": "x-anthropic-billing-header: cc_version=2.1.197.123; cc_entrypoint=sdk-cli;",
            },
            {
                "type": "text",
...
class TestReplaceSystemIdentity:
    ...
    def test_original_body_not_mutated(self):
        ...
        assert len(body["system"]) == original_len
    ...
    def test_drops_billing_header_even_when_not_first(self):
        body = {
            "system": [
                ...
                {
                    "type": "text",
                    "text": "x-anthropic-billing-header: cc_version=2.1.197;",
                },
