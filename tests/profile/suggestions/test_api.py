...
import trowel_py.profile.suggestions as profile_suggestions
from tests.profile.suggestions.support import suggestion
...
def test_public_functions_keep_signatures_and_module_identity() -> None:
    for name, signature in _FUNCTION_SIGNATURES.items():
        ...
        assert function.__module__ == "trowel_py.profile.suggestions"


def test_public_module_keeps_direct_symbols_used_by_callers() -> None:
    expected = {
...
def test_codec_dependencies_are_read_at_call_time(
...
def test_serializer_patch_flows_to_writer(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
...
def test_append_read_write_remain_inside_module_lock(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    events: list[str] = []
...
def test_lock_uses_exclusive_then_releases_and_closes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_lock_is_noop_without_fcntl(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
