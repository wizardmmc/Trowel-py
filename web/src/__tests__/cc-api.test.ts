...
describe("cc REST client", () => {
  ...
  it("listActiveSessions returns the active list + active_id (slice-028 D2)", async () => {
    vi.stubGlobal(
      ...
      mockFetchEnvelope({
        sessions: [
          { id: "s1", workdir: "/wd", model: "glm-5.2", name: "trowel-py", running: true },
          { id: "s2", workdir: "/wd", model: "glm-5.2", name: "trowel-py #2", running: false },
        ],
        active_id: "s1",
      }),
    );
  ...
  it("activateSession POSTs to /sessions/:id/activate (slice-028 D2)", async () => {
    ...
    expect(call?.[0]).toBe("/api/cc/sessions/s2/activate");
    expect(call?.[1]?.method).toBe("POST");
  });
