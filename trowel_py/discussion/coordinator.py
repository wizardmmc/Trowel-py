...
class DiscussionCoordinator:
    ...
    async def _consume_turn(
        ...
        *,
        ...
        """只接受当前根 turn 的 finished，静默 EOF 和旧/子终态都算失败。
        ...
        try:
            async for event in stream:
                ...
                if event_type == "finished" and matches_root:
                    ...
                    audit_events.append({"type": "finished", "turn_id": root_turn_id})
                    break
                if event_type == "interrupted" and matches_root:
                    ...
                    break
                if event_type == "error" and matches_root:
                    status, code, message = _classify_terminal_error(event)
                    terminal = (status, code, message)
                    audit_events.append(
                        {"type": "error", "turn_id": root_turn_id, "code": code}
                    )
                    break
                if event_type == "session_exited" and root_turn_id is not None:
                    terminal = (
                        "host_lost",
                        "HOST_EXITED",
                        "参与者运行工具在回答完成前退出",
                    ...
                    break
                if event_type == "host_status" and root_turn_id is not None:
                    payload = event.get("payload")
                    host_status = (
                        payload.get("status") if isinstance(payload, dict) else None
                    )
